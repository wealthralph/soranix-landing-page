  ...
  const groups = data.map((group) => {
    const links = group.links.map((link, index) => (
      ...
      >
        <Group gap={"xs"} mb={3}>
          ...
          {link.badge && (
            <Badge variant="light" radius={"sm"} size="xs">
              {link.badge}
            </Badge>
          )}
        </Group>
    ...
    return (
      <Grid.Col span={{ base: 6, sm: 4, md: 4, lg: 3 }} key={group.title}>
        <Text size="sm" mb={3} fw={500} className={classes.title}>
          {group.title}
        </Text>
        {links}
      </Grid.Col>
    ...
    <footer className={classes.footer}>
      ...
      <Container size={"xl"}>
        <Text size="sm" c={"dimmed"}>
          Soranix is a financial technology company, not a bank. Banking
          services are provided by licensed banking partners.
        </Text>
      </Container>
    </footer>
  );
